...
--
-- CS removed: intended for single-device SPI buses where CS is tied low
-- permanently on the PCB (e.g. ADS124S08 with CS → GND).
--------------------------------------------------------------------------------

entity spi_master is
  generic (
    CLK_FREQ  : integer   := 12_000_000; -- system clock frequency (Hz)
    SCLK_FREQ : integer   :=  2_000_000; -- desired SCK frequency   (Hz)
    DATA_W    : integer   := 8;          -- transaction width in bits
    CPOL      : std_logic := '0';        -- '0' = SCK idles low,  '1' = idles high
    CPHA      : std_logic := '1'         -- '0' = sample-first,   '1' = shift-first
  );
  port (
    clk      : in  std_logic;
    ...
    mosi     : out std_logic;
    miso     : in  std_logic
    -- cs_n removed: CS is tied permanently low on the PCB
  );
end spi_master;
...
begin
  ...
  sclk <= sclk_r when state = TRANSFER else CPOL;
        ...
        when TRANSFER =>
          ...
          if timer = HALF_PER - 1 then
            ...
            if sclk_r = CPOL then
              ...
              -- Sample MISO before checking done so the last bit is
